...
class NewsletterController extends DocumentControllerBase
{
    ...
    public function checksqlAction(Request $request): JsonResponse
    {
        ...
        try {
            if ($class = $request->request->get('class')) {
                $className = '\\OpenDxp\\Model\\DataObject\\' . ucfirst($class) . '\\Listing';
    ...
    public function sendAction(Request $request, MessageBusInterface $messengerBusOpenDxpCore): JsonResponse
    {
        $this->checkPermission('newsletters');
        ...
        $messengerBusOpenDxpCore->dispatch(
            new OpenDxp\Bundle\NewsletterBundle\Messenger\SendNewsletterMessage($document->getTmpStoreId(), \OpenDxp\Tool::getHostUrl())
        );
