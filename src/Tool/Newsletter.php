...
use OpenDxp\Bundle\NewsletterBundle\Event\DocumentEvents;
use OpenDxp\Bundle\NewsletterBundle\Model\Document\Newsletter as NewsletterDocument;
use OpenDxp\File;
use OpenDxp\Logger;
use OpenDxp\Mail;
use OpenDxp\Model;
...
class Newsletter
{
    ...
    public static function prepareMail(
        ...
        $config = OpenDxp::getContainer()->getParameter('opendxp_newsletter');
    ...
    public static function sendNewsletterDocumentBasedMail(Mail $mail, SendingParamContainer $sendingContainer): void
    {
        ...
        if (!empty($mailAddress)) {
            ...
            if ($config['use_specific']) {
                $mail->getHeaders()->addTextHeader('X-Transport', 'opendxp_newsletter');
            }
            ...
            OpenDxp::getEventDispatcher()->dispatch($event, DocumentEvents::NEWSLETTER_PRE_SEND);
            $mail->sendWithoutRendering($mailer);
            OpenDxp::getEventDispatcher()->dispatch($event, DocumentEvents::NEWSLETTER_POST_SEND);
