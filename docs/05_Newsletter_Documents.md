...
the namespace `opendxp.document.newsletters.addressSourceAdapters`, named like the adapter (e.g. `opendxp.document.newsletters.addressSourceAdapters.myAdapter`)
 and implement the methods `initialize`, `getName`, `getLayout` and `getValues`. As sample see [csvList](https://github.com/open-dxp/newsletter-bundle/blob/1.x/public/js/document/newsletters/addressSourceAdapters/csvList.js)
...
the email addresses. It needs to implement the interface `AddressSourceAdapterInterface`. As sample see
 [csvList](https://github.com/open-dxp/newsletter-bundle/blob/1.x/src/Document/Newsletter/AddressSourceAdapter/CsvList.php).
