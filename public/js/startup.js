opendxp.registerNS("opendxp.bundle.newsletter.startup");

opendxp.bundle.newsletter.startup = Class.create({
    type: 'newsletter',

    initialize: function () {
        document.addEventListener(opendxp.events.prepareDocumentTreeContextMenu, this.onPrepareDocumentTreeContextMenu.bind(this));
    },
    ...
    onPrepareDocumentTreeContextMenu: function (e) {
        ...
        if(childSupportedDocument && document.data.permissions.create && !opendxp.helpers.documentTypeHasSpecificRole(document.data.type, "only_printable_childrens")) {
            ...
            if (addBlankNewsletter) {
                ...
                documentMenu.newsletter.push({
                    text: "&gt; " + t("blank"),
                    iconCls: "opendxp_icon_newsletter opendxp_icon_overlay_add",
        ...
        Ext.MessageBox.prompt(textKeyTitle, textKeyMessage, function (tree, record, type, docTypeId, button, value) {
            if (button == "ok") {
                if (value) {
                    ...
                    if(opendxp.elementservice.isDisallowedDocumentKey(record.id, value)) {
                        return;
                    }

                    let params = {
                        key: opendxp.helpers.getValidFilename(value, "document"),
                        type: type,
                        docTypeId: docTypeId,
                        sourceTree: tree,
    ...
    populatePredefinedDocumentTypes: function(documentMenu, tree, record) {
        let me = this;
        let document_types = opendxp.globalmanager.get("document_types_store");
        document_types.sort([
...
const opendxpBundleNewsletter = new opendxp.bundle.newsletter.startup();
